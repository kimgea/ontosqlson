...
class TextField(SchemaFieldBase):
    def __init__(self, field_name=None, default=None, many=False, max_length=None, validators=None, *args, **kwargs):
        super().__init__([TextFieldType(max_length=max_length, validators=validators)],
                         default=default, field_name=field_name, many=many, *args, **kwargs)


class IntegerField(SchemaFieldBase):
    def __init__(self, field_name=None, default=None, many=False, validators=None, *args, **kwargs):
        super().__init__([IntegerFieldType(validators=validators)],
                         default=default, field_name=field_name, many=many, *args, **kwargs)


class PositiveIntegerField(SchemaFieldBase):
    def __init__(self, field_name=None, default=None, many=False, validators=None, *args, **kwargs):
        super().__init__([PositiveIntegerFieldType(validators=validators)],
                         default=default, field_name=field_name, many=many, *args, **kwargs)


class RelationField(SchemaFieldBase):
    def __init__(self, range_type, field_name=None, default=None, many=False, validators=None, *args, **kwargs):
        self._ensure_range_type_is_valid(range_type)
        in_range_type = self._get_range_field_type(range_type, validators)
    ...
    @staticmethod
    def _get_range_field_type(range_type, validators):
        if not isinstance(range_type, RelationFieldType):
            return RelationFieldType(range_type, validators=validators)
        return range_type


class MixField(SchemaFieldBase):
    def __init__(self, range_includes, field_name=None, default=None, many=False, validators=None, *args, **kwargs):
        if not range_includes:
            raise ValueError("Missing value: range_includes can not be empty")
        new_ranges = self._clean_range_includes(range_includes, validators)
        super().__init__(new_ranges, default=default, field_name=field_name, many=many, *args, **kwargs)

    def _clean_range_includes(self, range_includes, validators):
        new_ranges = []
        for check_range in range_includes:
            check_range = self._get_range_field_type(check_range, validators)
